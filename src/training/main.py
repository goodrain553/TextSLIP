...
from training.train import train_one_epoch, evaluate, train_one_epoch_ex
from training import train


def save_checkpoint(model, optimizer, scaler, completed_epoch, args, loss_fn=None):
    checkpoint_dict = {
...
def main(args=None):
    ...
    use_tslip = hasattr(args, "use_tslip") and args.use_tslip
    ...
    # Set momentum coefficient if momentum encoder exists
    if hasattr(model, 'momentum_encoder') and model.momentum_encoder is not None:
        if hasattr(args, 'momentum_coef'):
            model.momentum_coef = args.momentum_coef
            if is_master(args):
                logging.info(f"Set momentum coefficient to {args.momentum_coef}")
    ...
    if args.train_data:
        ...
        # Select weight decay based on loss type
        if hasattr(args, 'loss') and args.loss == 'SLIPLoss':
            # Use SLIP-specific weight decay if provided, otherwise fall back to regular wd
            weight_decay = args.wd_slip if args.wd_slip is not None else args.wd
            logging.info(f"Using SLIP weight decay: {weight_decay}")
        else:
            weight_decay = args.wd
            logging.info(f"Using CLIP weight decay: {weight_decay}")
    ...
    # Create persistent loss function for queue-based methods (e.g., ESimCSE)
    loss_fn = None
    if hasattr(args, "engine") and args.engine == "train_one_epoch_textSLIP":
        ...
        # Load loss state from checkpoint if resuming
        if args.resume and os.path.isfile(args.resume):
            checkpoint = torch.load(args.resume, map_location='cpu')
            if 'loss_state' in checkpoint:
                loss_fn.load_state_dict(checkpoint['loss_state'])
                if is_master(args):
                    logging.info("Loaded loss state (including queue) from checkpoint")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if hasattr(args, "engine"):
            ...
            # Pass tokenizer for text augmentation if using train_one_epoch_textSLIP
            if engine == "train_one_epoch_textSLIP":
                engine_cls(model, data, epoch, epoch_step, optimizer, scaler, scheduler, args, writer, loss_fn=loss_fn, tokenizer=tokenize)
            elif engine == "train_one_epoch_ex":
                engine_cls(model, data, epoch, epoch_step, optimizer, scaler, scheduler, args, writer)
            else:
                engine_cls(model, data, epoch, epoch_step, optimizer, scaler, scheduler, args, writer, loss_fn=loss_fn)
        ...
        save_checkpoint(model, optimizer, scaler, completed_epoch, args, loss_fn=loss_fn)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python src/training/main.py <config_name> <log_dir> [pretrained_checkpoint]")
        sys.exit(1)
    config = search_config(sys.argv[1])
    config.logs = sys.argv[2]
    if len(sys.argv) >= 4:
        config.pretrained = sys.argv[3]
