...
from .transform import image_transform, image_transform_slip
...
def create_model(
    ...
    if pretrained.lower() == 'openai':
        ...
        if use_slip and not hasattr(model, 'simclr_projection'):
            print("no projection module")
            pass
        ...
        model_cfg['use_slip'] = use_slip
        model_cfg['use_tslip'] = use_tslip
        model_cfg['simclr_proj_dim'] = simclr_proj_dim
...
def create_model_and_transforms(
    ...
    model = create_model(
        ...
        use_slip = use_slip,
        use_tslip = use_tslip,
        simclr_proj_dim = simclr_proj_dim,
    )
    if use_slip:
        preprocess_train = image_transform_slip(model.visual.image_size, is_train=True, mean=mean, std=std, inmem=inmem)
